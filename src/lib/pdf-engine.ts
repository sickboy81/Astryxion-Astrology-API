...
export async function generatePDF(
  ...
  try {
    // `networkidle0` waits for zero network for 500ms — Google Fonts @import in SHARED_STYLES
    // often never satisfies that in serverless/restricted egress, and natal PDFs are large enough
    // to hit default timeouts. `load` is sufficient for print layout.
    await page.setContent(htmlContent, { waitUntil: "load", timeout: 120_000 });
...
export function createMiniReportHTML(
  ...
  const { input, planets: planetsRaw } = data;
...
<body>
  <div class="pdf-page">
    ...
    <div class="details-grid">
      ...
      <div class="details-item"><span class="details-label">${t.time}:</span> ${input.timeUtc || input.time || "12:00:00"}</div>
    </div>
...
export function createNatalReportHTML(
  ...
  const loc = resolvePdfLang(lang);
  const t = TRANSLATIONS[loc] || TRANSLATIONS.en;
  const { input, planets: planetsRaw, houses } = data;
  const planets = Array.isArray(planetsRaw) ? planetsRaw : [];
  const verificationId = crypto.randomUUID().slice(0, 8).toUpperCase();
  const qrSvg = generateQRCodeSVG(`https://astryxion.com/verify/${verificationId}`, 80);
