            ...
            try {
                ...
                data.tiers.forEach(p => {
                    ...
                    tr.innerHTML = \`
                        ...
                        <td>\${p.price}</td>
                        <td>
                            <button class="btn btn-primary" onclick="openPlanModal('\${p.id}', '\${p.displayName}', \${p.requestsPerDay}, '\${p.price}')">Edit Plan</button>
                        </td>
                    \`;
                    tbody.appendChild(tr);
                });
        ...
        async function savePlan() {
            ...
            try {
                await api('/admin/api/config/tiers', {
                    method: 'POST',
                    body: JSON.stringify({ id, displayName, requestsPerDay, price })
                });
