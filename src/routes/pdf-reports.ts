...
export async function registerPDFRoutes(app: FastifyInstance) {
  ...
  app.post("/api/v1/reports/natal-pdf", {
    ...
    try {
      ...
      if (!data?.input || !Array.isArray(data.planets)) {
        app.log.error(
          { keys: data && typeof data === "object" ? Object.keys(data) : [] },
          "Natal PDF: unexpected /natal-chart response shape"
        );
        return reply.code(502).send(
          errorBody("BAD_UPSTREAM", "Natal chart data incomplete. Please retry or contact support.")
        );
      }
